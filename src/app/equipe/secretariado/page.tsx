...
export default function SecretariadoPage() {
  const membros = dadosMembros.sec;
      ...
      <div className="max-w-6xl mx-auto mb-10 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            A instância máxima de deliberação e coordenação executiva do SenaMUN V.
          </p>
        </div>
      </div>
        ...
        {membros.map((membro, index) => (
          <div key={index} className="bg-white dark:bg-slate-800 shadow-lg hover:scale-105 hover:-translate-y-1 transition duration-300 flex flex-col items-center text-center p-4 rounded-xl">
            <div className="relative w-full aspect-square mb-4">
              <Image src={`/time/secretarios/${membro.imagem}`} alt={membro.nome} fill className="object-cover rounded-xl" />
            </div>
